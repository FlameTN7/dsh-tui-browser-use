...
try {
  ...
  assert.equal(registered.length, 19, '19 browser tools registered')
  ...
  if (process.env.DSH_TUI_SKIP_BROWSER) {
    ...
    console.log('[container] dsh-tui-browser-use container check OK: 19 tools registered (browser skipped)')
    process.exit(0)
  }
  ...
  console.log('[container] dsh-tui-browser-use container check OK: 19 tools registered, browser live (version=' + statusRes.value.version + ')')
